//GET: /api/v2/company/setting/pipeline
...
test.describe("/api/v2/company/setting/pipeline GET requests @company", async () => {
    let pipeline: any;

    test.beforeAll(async () => {
        await deleteAllPipelines(authObjects.companyOneAuthHeaders);
        pipeline = await createNewPipeline(authObjects.companyOneAuthHeaders);
    });

    test("GET with valid credentials @happy", async ({ request }) => {
        const response = await request.get(`/api/v2/company/setting/pipeline`, {
        ...
        expect(body.data[1].name).toBe(pipeline.name);
