//GET: /api/v2/screening-question-set/{id}

import { test, expect } from '@playwright/test';
import authObjects from '@datafactory/auth';
import { createAssertions } from "@helpers/createAssertions";
import { createQuestionSet, deleteAllQuestionSets, getQuestionSetById, getScreeningQuestionSetQuestions } from '@datafactory/question-group';

test.describe("/api/v2/screening-question-set/{id} GET requests @company", async () => {
    ...
    test("GET with valid credentials @happy", async () => {
        ...
        let response_questions = await getScreeningQuestionSetQuestions(authObjects.companyOneAuthHeaders, question_set.id);
    ...
    test("GET with invalid credentials but valid id", async ({ request }) => {
        const response = await request.get(`/api/v2/screening-question-set/${question_set.id}`, {
            headers: {
    ...
    test("GET with valid credentials but invalid id", async ({ request }) => {
        const response = await request.get(`/api/v2/screening-question-set/123`, {
