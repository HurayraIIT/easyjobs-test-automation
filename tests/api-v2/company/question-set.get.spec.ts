//GET: /api/v2/question-set/{id}

import { test, expect } from '@playwright/test';
import authObjects from '@datafactory/auth';
import { createAssertions } from "@helpers/createAssertions";
import { createQuestionSet, deleteAllQuestionSets, getQuestionSetById, getQuestionSetQuestions, getQuizMetaData, QuestionSetType } from '@datafactory/question-group';

test.describe("/api/v2/question-set/{id} GET requests @company", async () => {
    ...
    test("GET with valid credentials @happy", async () => {
        ...
        let response_questions = await getQuestionSetQuestions(authObjects.companyOneAuthHeaders, question_set.id);
